...
connect().then(async (mongoClient: MongoClient | null) => {
  if (mongoClient) {
    ...
    const ro: InsertOneWriteOpResult<any> = await db.collection('inserts').insertOne({ a: 1 });
    console.log('ro.insertedCount: ', ro.insertedCount);

    // insert multiple documents
    const rm: InsertWriteOpResult<any> = await db.collection('inserts').insertMany([{ a: 2 }, { a: 3 }]);
